...
void SCDWrapper::restart(int & iStep, double & advTime)
{
    ...
    */
    ...
    if (ofile.good()) {
        while (getline(ofile, oneLine)) {
            ...
            if (step == 1) {
                ...
                for (int i = 0; i < POINTS; i++) {
                    lineHold >> number[i];
                    number[i] = (int)(DENSITY*VOLUME/1e+6*90.0);
                }
    ...
    ofile.close();
    objectKey = 1;
    number[0] = (int)(DENSITY*VOLUME/1e+6*2.0);
    addNewObjectToMap(objectKey, number);
    objectKey = 2;
    number[0] = (int)(DENSITY*VOLUME/1e+6*1.0);
    addNewObjectToMap(objectKey, number);
...
void SCDWrapper::sizeDistribution(double& time){
    ...
    for (iter = allObjects.begin(); iter != allObjects.end(); ++iter){
        ...
        if(attribute > 0){
            // it is SIA cluster
            size = pow(ATOMICVOLUME*1e-21*abs(attribute)/PI/BURGER, 0.5); //[cm]
            //lambda = 0.15;
            lambda = 0.2;
            ...
            // it is V cluster
            size = pow(3*ATOMICVOLUME*abs(attribute)/4/PI, 0.333)*1e-7; //[cm]
            /*
            if(size >= 1e-7){
                Vc += totalNumber;
            }
...
double SCDWrapper::getTotalDpa(double& time){
    double dpa_rate = 0.000043;
    double dpa = dpa_rate*time;
    return dpa;
}

int SCDWrapper::doClusterDynamics(double& oneOverL, double& dt){
    int64 theOtherKey = 0;
